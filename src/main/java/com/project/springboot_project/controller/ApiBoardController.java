...
public class ApiBoardController  extends ApiController{
    ...
    @GetMapping("/list")
    public ResponseEntity<StatusMessage> list(){
        jsonHeader();
    ...
    @PostMapping("/update")
    public ResponseEntity<StatusMessage> update(CreateBoardDto createBoardDto){
        jsonHeader();
        Optional<BoardEntity> result = apiBoardService.update(createBoardDto);
        if(result.isPresent()){
            message.setData(result.get());
            message.setMessage(StatusEnum.OK.getStatusMessage());
            message.setStatus(StatusEnum.OK.getStatusCode());
